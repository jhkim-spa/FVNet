...
class FVNetAnchorHead(nn.Module, AnchorTrainMixin):
    ...
    def _init_layers(self):
        ...
        self.cls_out_channels = self.num_anchors * self.num_classes\
            * self.anchor_generator.num_bins
        self.reg_out_channels = self.num_anchors * self.box_code_size\
            * self.anchor_generator.num_bins
        self.dir_cls_out_channels = self.num_anchors * 2\
            * self.anchor_generator.num_bins
        self.conv_cls = nn.Conv2d(self.feat_channels, self.cls_out_channels, 1)
        self.conv_reg = nn.Conv2d(self.feat_channels, self.reg_out_channels, 1)
        if self.use_direction_classifier:
            self.conv_dir_cls = nn.Conv2d(self.feat_channels,
                                          self.dir_cls_out_channels, 1)
