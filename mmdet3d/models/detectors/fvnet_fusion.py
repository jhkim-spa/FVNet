...
class FVNetFusion(SingleStage3DDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(FVNetFusion, self).__init__(
            backbone=backbone_fv,
            neck=neck,
            bbox_head=bbox_head,
            train_cfg=train_cfg,
            test_cfg=test_cfg,
        ...
        self.backbone_fv = self.backbone
        self.backbone_img = build_backbone(backbone_img)

    def extract_feat(self, fv, img):
        ...
        if self.with_neck:
            feats_img = self.neck_img(feats_img)
        ...
        device = fv[0].device
        mlvl_valid_coords = []
        featmap_sizes = [featmap.size()[-2:] for featmap in feats_fv]
        fv_src = fv

        for i in range(len(feats_fv)):
        ...
        return feats_fv, feats_img, mlvl_valid_coords
    ...
    def forward_train(self,
        ...
        feats_fv, feats_img, valid_coords = self.extract_feat(fv, img)
        feats = [torch.cat((feat_fv, feat_img), dim=1)\
            for feat_fv, feat_img in zip(feats_fv, feats_img)]
